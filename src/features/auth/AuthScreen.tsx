...
import { Sparkles, Mail, Lock, User, ArrowRight, AlertCircle, KeyRound, Loader2 } from 'lucide-react';
import { useAuth } from './AuthContext';

// Official Google & GitHub Brand SVGs
const GoogleIcon = () => (
...
function formatFirebaseError(err: any): string {
  ...
  if (code === 'auth/operation-not-allowed') {
    return 'GitHub sign-in is not enabled in your Firebase Console. Please use Google or Email.';
  }
...
export const AuthScreen: React.FC<{ initialTab?: 'login' | 'register'; onAuthenticated?: () => void }> = ({
  ...
  return (
    <div className="w-full max-w-sm mx-auto p-6 md:p-8 bg-white rounded-3xl shadow-xl border border-slate-100">
      ...
      {/* Primary OAuth Actions: Google & GitHub */}
      <div className="grid grid-cols-2 gap-2.5 mb-5">
        <button
          type="button"
          disabled={submitting}
          onClick={() => handleOAuth('google')}
          className="flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-xs font-bold text-slate-700 transition shadow-xs cursor-pointer disabled:opacity-60"
        >
          <GoogleIcon />
          <span>Google</span>
        </button>
